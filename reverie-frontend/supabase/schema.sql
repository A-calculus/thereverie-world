...
-- MIGRATION SNAPSHOT
-- ============================================================
-- Preserve existing Phase 2 rows in session-local temporary backup tables,
-- then drop and recreate the public tables/policies/types below. New-only
-- columns are restored as NULL/defaults when the previous schema did not
-- have them. The temporary backup tables are dropped explicitly after restore.
...
BEGIN
  ...
  LOOP
    IF to_regclass(format('public.%I', table_name)) IS NOT NULL THEN
      EXECUTE format('DROP TABLE IF EXISTS pg_temp.%I', '_reverie_backup_' || table_name);
...
DO $$
BEGIN
  IF to_regclass('pg_temp._reverie_backup_users') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_templates') IS NOT NULL THEN
    EXECUTE $sql$
...
DO $$
BEGIN
  IF to_regclass('pg_temp._reverie_backup_agents') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_tools') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_tool_secrets') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_tool_mcp_capabilities') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_user_sdk_agents') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_worlds') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_triggers') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_events') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_world_deployments') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_world_reactivity_subscriptions') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_world_runtime_runs') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_world_agent_requests') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_world_balance_snapshots') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_world_reconciliation_checkpoints') IS NOT NULL THEN
    EXECUTE $sql$
  ...
  IF to_regclass('pg_temp._reverie_backup_world_secrets') IS NOT NULL THEN
    EXECUTE $sql$
...
DECLARE
  ...
  backup_tables TEXT[] := ARRAY[
    '_reverie_backup_users',
    '_reverie_backup_templates',
    '_reverie_backup_agents',
    '_reverie_backup_tools',
    '_reverie_backup_tool_secrets',
    '_reverie_backup_tool_mcp_capabilities',
    '_reverie_backup_user_sdk_agents',
    '_reverie_backup_worlds',
    '_reverie_backup_triggers',
    '_reverie_backup_events',
    '_reverie_backup_world_deployments',
    '_reverie_backup_world_reactivity_subscriptions',
    '_reverie_backup_world_runtime_runs',
    '_reverie_backup_world_agent_requests',
    '_reverie_backup_world_balance_snapshots',
    '_reverie_backup_world_reconciliation_checkpoints',
    '_reverie_backup_world_secrets'
  ];
BEGIN
  FOREACH backup_table IN ARRAY backup_tables
  LOOP
    EXECUTE format('DROP TABLE IF EXISTS pg_temp.%I', backup_table);
  END LOOP;
END $$;
